    ...
    try:
        ...
        for rec in records:
            ...
            if isinstance(mes_raw, list):
                ...
                mes_str = str(mes_raw)
            out.append({"id": rec["id"], "nome": f.get("Fornecedor", f.get("Nome", f.get("Descrição", ""))), "cat": f.get("Categoria", f.get("Tipo Despesa", "")), "valor": float(f.get("Valor", 0) or 0), "mes": mes_str, "pago": bool(f.get("Pago?", False)), "fatura": bool(f.get("Fatura Recebida?", False) or f.get("Fatura?", False)), "notas": f.get("Notas", ""), "tipo": "variavel"})
        return jsonify({"success": True, "records": out})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_wa_templates():
    ...
    try:
        ...
        for rec in records:
            f = rec.get("fields", {})
            out.append({"id": rec["id"], "name": f.get("Name", ""), "label": f.get("Name", ""), "categoria": f.get("Categoria", ""), "empresa": f.get("Empresa", ""), "topico": f.get("Tópico", ""), "contexto": f.get("Contexto", ""), "msg_en": f.get("Message 🇬🇧", ""), "msg_pt": f.get("Mensagem 🇵🇹", ""), "msg_fr": f.get("Mensagem 🇫🇷", ""), "subj_en": f.get("Subject 🇬🇧", ""), "subj_pt": f.get("Assunto Email 🇵🇹", f.get("Assunto 🇵🇹", "")), "subj_fr": f.get("Assunto 🇫🇷", ""), "com_cancel": f.get("Com cancelamento", ""), "vezes": f.get("Vezes Enviado", 0), "text_pt": f.get("Mensagem 🇵🇹", f.get("Message 🇬🇧", "")), "text_en": f.get("Message 🇬🇧", ""), "subject_pt": f.get("Assunto Email 🇵🇹", f.get("Assunto 🇵🇹", "")), "subject_en": f.get("Subject 🇬🇧", "")})
        return jsonify({"success": True, "records": out})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_extrato_parceiros():
    ...
    try:
        ...
        formula = f'{{Mes}} = "{mes}"' if mes else None
...
def criar_extrato_mes():
    ...
    try:
        ...
        if not mes_str: return jsonify({"error": "mes obrigatório"}), 400
        parts = mes_str.split(" ")
        if len(parts) != 2 or not parts[1].isdigit(): return jsonify({"error": f"Formato inválido: {mes_str}"}), 400
        mes_nome, ano = parts[0], int(parts[1])
        mes_num = MESES_IDX.get(mes_nome)
        if not mes_num: return jsonify({"error": f"Mês inválido: {mes_nome}"}), 400
        first_day = date(ano, mes_num, 1)
        last_day  = date(ano, mes_num, calendar.monthrange(ano, mes_num)[1])
        ...
        for rec in airtable_list_table(BASE_RESERVAS, TAB_AT_ID):
            ...
            if parceiro and parceiro not in parceiros_com_reservas:
                ...
                elif any(k in ativ for k in ["buggy","quad","zip","adventure","canyon"]): tipo = "Adventure"
        ...
        for rec in existing:
            ...
            if mes_rec == mes_str and parc: existing_map[parc] = rec
        created = []; skipped = []
        for parceiro, tipo in parceiros_com_reservas.items():
            if parceiro in existing_map: skipped.append(parceiro); continue
            airtable_create(BASE_EXTRATO, TAB_EXTRATO, {"Parceiro": parceiro, "Mês": mes_str, "Tipo": tipo})
...
def gerar_extrato_parceiro():
    ...
    try:
        ...
        if not parceiro or not mes_str: return jsonify({"error": "parceiro e mes obrigatórios"}), 400
        meses_list = d.get("meses", []) or [mes_str]
        parsed_months = []
        for m_str in meses_list:
            parts = m_str.strip().split(" ")
            if len(parts) != 2 or not parts[1].isdigit(): continue
            m_num = MESES_IDX.get(parts[0])
            if m_num: parsed_months.append((parts[0], int(parts[1]), m_num))
        if not parsed_months: return jsonify({"error": f"Nenhum mês válido em: {meses_list}"}), 400
...
def gerar_extratos_mes():
    ...
    try:
        ...
        mes_str = d.get("mes","").strip()
        if not mes_str: return jsonify({"error": "mes obrigatório"}), 400
        formula = f'{{Mes}} = "{mes_str}"'
        registos = airtable_list_table(BASE_EXTRATO, TAB_EXTRATO, formula=formula)
        if not registos: return jsonify({"success":True,"results":[],"message":f"Nenhum parceiro em {mes_str}"})
        results = []
        for reg in registos:
            f   = reg.get("fields", {})
...
def enviar_extrato_email():
    ...
    try:
        ...
        if not to: return jsonify({"error": "Email destinatario obrigatorio"}), 400
        gmail_user = os.environ.get("GMAIL_USER","")
        gmail_pass = os.environ.get("GMAIL_APP_PASSWORD","")
        if not gmail_user or not gmail_pass: return jsonify({"error": "Gmail nao configurado."}), 500
        msg = MIMEMultipart()
        ...
        if record_id and record_id.startswith("rec"):
            try: airtable_patch(BASE_EXTRATO, TAB_EXTRATO, record_id, {"Mail enviado / pedido?": True})
            except: pass
...
def enviar_voucher_email():
    ...
    try:
        ...
        gmail_user = os.environ.get("GMAIL_USER","")
        gmail_pass = os.environ.get("GMAIL_APP_PASSWORD","")
        if not gmail_user or not gmail_pass: return jsonify({"error": "Gmail nao configurado no Railway"}), 500
        ...
        with smtplib.SMTP_SSL("smtp.gmail.com", 465, context=ssl.create_default_context()) as server:
            ...
            server.sendmail(gmail_user, to, msg.as_string())
        if record_id and record_id.startswith("rec"):
            try: airtable_patch(BASE_RESERVAS,"Atividades",record_id,{"Email Enviado":True}); cache_clear("at")
            except: pass
...
def api_chat():
    ...
    try:
        ...
        if not anthropic_key: return jsonify({"error": "ANTHROPIC_API_KEY nao configurada no Railway"}), 500
        body = request.get_json() or {}
...
def wz_messages():
    if not check_key(): return jsonify({"error": "Unauthorized"}), 401
    chat_id = request.args.get("chatId",""); chat_type = request.args.get("chatType","whatsapp"); limit = request.args.get("limit",20)
...
def wz_iframe():
    ...
    try:
        ...
        user_id = b.get("userId","milton"); user_name = b.get("userName","Milton")
        wazzup_req("POST", "/users", [{"id":user_id,"name":user_name}])
        s, d = wazzup_req("POST", "/iframe", {"user":{"id":user_id,"name":user_name},"scope":"global","options":{"useDealsEvents":True,"useMessageEvents":True}})
        return jsonify(d), s
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
GMAIL_CLIENT_SECRET = os.environ.get("GMAIL_CLIENT_SECRET","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")
...
def gmail_get_tokens():
    global _gmail_token_cache
    if _gmail_token_cache: return _gmail_token_cache
    try:
    ...
    if raw:
        try: _gmail_token_cache = json.loads(raw); return _gmail_token_cache
        except: pass
    return None

def gmail_refresh(tokens):
    data = urllib.parse.urlencode({"client_id":GMAIL_CLIENT_ID,"client_secret":GMAIL_CLIENT_SECRET,"refresh_token":tokens["refresh_token"],"grant_type":"refresh_token"}).encode()
...
def gmail_auth():
    params = urllib.parse.urlencode({"client_id":GMAIL_CLIENT_ID,"redirect_uri":GMAIL_REDIRECT_URI,"response_type":"code","scope":GMAIL_SCOPES,"access_type":"offline","prompt":"consent"})
    url = "https://accounts.google.com/o/oauth2/v2/auth?" + params
    return f'''<!DOCTYPE html><html><body style="font-family:sans-serif;padding:40px;max-width:500px;margin:auto">
    <h2 style="color:#0a8f82">Beyond Madeira — Ligar Gmail</h2>
    <a href="{url}" style="display:inline-block;padding:14px 28px;background:#0a8f82;color:white;border-radius:8px;text-decoration:none;font-size:16px;font-weight:bold">Autorizar com Google</a>
    </body></html>'''

@app.route("/gmail/callback")
def gmail_callback():
    code = request.args.get("code")
    if not code: return "Erro: sem código de autorização", 400
    data = urllib.parse.urlencode({"code":code,"client_id":GMAIL_CLIENT_ID,"client_secret":GMAIL_CLIENT_SECRET,"redirect_uri":GMAIL_REDIRECT_URI,"grant_type":"authorization_code"}).encode()
    req = urllib.request.Request("https://oauth2.googleapis.com/token", data=data, method="POST")
    req.add_header("Content-Type","application/x-www-form-urlencoded")
    try:
        with urllib.request.urlopen(req) as r:
...
def gmail_inbox():
    ...
    if not tokens: return jsonify({"error": "Gmail não autenticado. Vai a /gmail/auth"}), 401
    try:
        tab = request.args.get("tab","INBOX"); limit = int(request.args.get("limit",20)); q = request.args.get("q","")
        query = q if q else ""
        ...
        for m in messages[:limit]:
            ...
            result.append({"id":msg["id"],"threadId":msg["threadId"],"subject":headers.get("Subject","(sem assunto)"),"from":headers.get("From",""),"to":headers.get("To",""),"date":headers.get("Date",""),"snippet":msg.get("snippet",""),"unread":"UNREAD" in labels,"starred":"STARRED" in labels})
...
def gmail_thread(thread_id):
    if not check_key(): return jsonify({"error": "Unauthorized"}), 401
    tokens = gmail_get_tokens()
    if not tokens: return jsonify({"error": "Gmail não autenticado"}), 401
    try:
        ...
        for msg in thread.get("messages",[]):
            ...
            messages.append({"id":msg["id"],"from":headers.get("From",""),"to":headers.get("To",""),"date":headers.get("Date",""),"subject":headers.get("Subject",""),"body":body,"unread":"UNREAD" in msg.get("labelIds",[])})
...
def gmail_send_api():
    ...
    try:
        ...
        b = request.get_json()
        msg = MIMEMultipart("alternative")
        msg["To"] = b.get("to",""); msg["Subject"] = b.get("subject","")
        thread_id = b.get("threadId","")
        if thread_id: msg["In-Reply-To"] = thread_id
        msg.attach(MIMEText(b.get("body",""), "html"))
...
def gmail_star():
    ...
    if not tokens: return jsonify({"error": "Gmail não autenticado"}), 401
    try:
        b = request.get_json(); msg_id = b.get("id"); action = b.get("action")
        ...
        gmail_request("POST", f"/messages/{msg_id}/modify", tokens, {"addLabelIds":add_labels,"removeLabelIds":remove_labels})
        return jsonify({"success": True})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

# ── DESPESAS COMBINADAS (novo) ────────────────────────────────
...
def get_financeiro_despesas():
    ...
    try:
        def _mes(f):
            ...
            return str(m)
        out = []
        for rec in airtable_list(BASE_FINANCEIRO, "Despesas Fixas"):
            f = rec.get("fields",{})
            out.append({"id":rec["id"],"nome":f.get("Fornecedor",f.get("Nome","")),"cat":f.get("Categoria",""),"valor":float(f.get("Valor Mensal",0)or 0),"mes":_mes(f),"pago":bool(f.get("Pago?",False)),"fatura":bool(f.get("Fatura Recebida?",False)or f.get("Fatura?",False)),"notas":f.get("Notas",""),"tipo":"fixa"})
        for rec in airtable_list(BASE_FINANCEIRO, "Despesas Variáveis"):
            f = rec.get("fields",{})
            out.append({"id":rec["id"],"nome":f.get("Fornecedor",f.get("Nome",f.get("Descrição",""))),"cat":f.get("Categoria",f.get("Tipo Despesa","")),"valor":float(f.get("Valor",0)or 0),"mes":_mes(f),"pago":bool(f.get("Pago?",False)),"fatura":bool(f.get("Fatura Recebida?",False)or f.get("Fatura?",False)),"notas":f.get("Notas",""),"tipo":"variavel"})
        return jsonify({"success": True, "records": out})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def patch_financeiro_despesa(record_id):
    ...
    try:
        ...
        if "valor" in body: fields["Valor (€)"]  = float(body["valor"])
        if "notas" in body: fields["Notas"]      = str(body["notas"])
        if not fields: return jsonify({"error": "Nenhum campo para actualizar"}), 400
...
def stripe_create_payment_link():
    if not check_key():
        return jsonify({"error": "Unauthorized"}), 401
    return jsonify({"error":"Stripe não configurado. Adiciona STRIPE_SECRET_KEY no Railway.","success":False}), 501
